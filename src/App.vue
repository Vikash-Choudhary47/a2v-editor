...
<template>
  ...
  <div v-else class="h-screen flex flex-col bg-white">
    ...
    <div
      v-if="store.error"
      class="fixed bottom-6 left-1/2 -translate-x-1/2 bg-red-50 text-red-600 px-6 py-3 rounded-lg border border-red-200 flex items-center gap-3 z-50 shadow-sm"
    >
      <span class="text-sm">{{ store.error }}</span>
      <button class="text-sm font-medium hover:text-red-800" @click="store.setError(null)">
        Dismiss
      </button>
    </div>
  </div>
