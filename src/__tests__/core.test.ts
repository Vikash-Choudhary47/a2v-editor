...
describe('HistoryManager', () => {
  ...
  it('clears redo stack on new command', () => {
    ...
    const cmd1 = { type: 'A', execute: () => { state = 1 }, undo: () => { state = 0 } }
    const cmd2 = { type: 'B', execute: () => { state = 2 }, undo: () => { state = 0 } }
...
describe('OverlayEngine', () => {
  ...
  it('creates text overlay with defaults', () => {
    const obj = engine.createTextOverlay({ page: 1, x: 10, y: 20, text: 'Hi' }) as AnyOverlay & { fontSize: number; color: string }
  ...
  it('creates shape overlay', () => {
    const obj = engine.createShapeOverlay({
      page: 1, x: 0, y: 0, width: 100, height: 50, shapeType: ShapeType.RECT,
    })
