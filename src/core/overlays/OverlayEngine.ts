...
export class OverlayEngine {
  addObject(document: PdfDocument, object: AnyOverlay): PdfDocument {
    return {
      ...
      pages: document.pages.map((p) =>
        p.pageNumber === object.page ? { ...p, overlays: [...p.overlays, object] } : p,
      ),
    }
  }
  ...
  updateObject(document: PdfDocument, id: string, updates: Partial<AnyOverlay>): PdfDocument {
    return {
      ...document,
      pages: document.pages.map((p) => ({
        ...p,
        overlays: p.overlays.map((o) => (o.id === id ? ({ ...o, ...updates } as AnyOverlay) : o)),
      })),
    }
  }
